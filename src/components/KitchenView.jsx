...
import { Clock, ChefHat, CheckCircle, XCircle, Bell } from 'lucide-react';
...
export function KitchenView() {
  const { orders, updateOrderStatus, isOnline, loaded } = useOrders();
  const [newOrdersCount, setNewOrdersCount] = useState(0);
  const seenIdsRef = useRef(new Set());
  const notifiedIdsRef = useRef(new Set());

  const kitchenOrders = useMemo(() => {
    const filtered = orders.filter(o => KITCHEN_STATUSES.includes(o.status));
    const sorted = [...filtered].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
    return sorted;
  }, [orders]);

  useEffect(() => {
    const pendingIds = kitchenOrders.filter(o => o.status === 'PENDING').map(o => o.id);
    let fresh = 0;
    pendingIds.forEach(id => {
      if (!seenIdsRef.current.has(id) && !notifiedIdsRef.current.has(id)) {
        notifiedIdsRef.current.add(id);
        fresh += 1;
      }
    });
    if (fresh > 0) {
      setNewOrdersCount(c => c + fresh);
  ...
  const dismissNewOrders = () => {
    setNewOrdersCount(0);
    seenIdsRef.current = new Set(kitchenOrders.map(o => o.id));
  };
  ...
  const grouped = kitchenOrders.reduce((acc, order) => {
        ...
        <div className="relative flex items-center gap-2">
          {newOrdersCount > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-amber-100 text-amber-800 border border-amber-300 font-bold text-sm animate-pulse"
            >
          ...
          {!isOnline && (
            <span className="text-xs font-bold text-red-600 bg-red-50 border border-red-200 px-2 py-1 rounded-lg">Offline</span>
          )}
        </div>
      </div>

      <div aria-live="polite" className="sr-only">
        {newOrdersCount > 0 ? `${newOrdersCount} pedido${newOrdersCount > 1 ? 's' : ''} nuevo${newOrdersCount > 1 ? 's' : ''} para preparar` : ''}
      </div>

      {!loaded && kitchenOrders.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {items.map(order => (
                    <div key={order.id} className={cn('card bg-white border-2 overflow-hidden', STATUS_COLORS[status].split(' ').pop())}>
                      <div className={cn('px-4 py-3 border-b flex items-center justify-between gap-2', STATUS_COLORS[status].split(' ').slice(0, 2).join(' '))}>
                        <div className="min-w-0">
                        ...
                        <span className="text-xs font-bold uppercase shrink-0">{STATUS_LABELS[status]}</span>
                      </div>
                      <div className="p-4 space-y-3">
                        {(order.items || []).map(item => (
                          <div key={item.id} className="flex justify-between items-center">
                            ...
                            {item.notes && <span className="text-xs text-gray-400 italic text-right max-w-[50%] break-words">{item.notes}</span>}
                          </div>
                        ))}
                      </div>
                      <div className="px-4 py-3 border-t bg-gray-50 flex gap-2">
                        {NEXT_STATUS[status] && (
                          <button onClick={() => advanceStatus(order.id, status)} className="flex-1 btn-primary text-sm py-2 min-h-11 flex items-center justify-center gap-2">
                            <CheckCircle className="w-4 h-4" />
                            {status === 'PENDING' ? 'Iniciar' : 'Marcar listo'}
                          </button>
                        )}
                        {status === 'PENDING' && (
                          <button onClick={() => cancelOrder(order.id)} className="px-4 min-h-11 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Cancelar pedido" aria-label="Cancelar pedido">
