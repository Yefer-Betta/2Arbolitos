...
export function Finance() {
    ...
    const salesByMethod = { cash_cop: 0, cash_usd: 0, nequi: 0, debit: 0 };
    currentOrders.forEach(order => {
        const method = order.payment?.method || 'cash_cop';
        if (method === 'cash_cop') salesByMethod.cash_cop += order.totalCop;
        else if (method === 'cash_usd') salesByMethod.cash_usd += order.totalUsd;
        else if (method === 'nequi') salesByMethod.nequi += order.totalCop;
        else salesByMethod.debit += order.totalCop;
    });

    // Filtered sales for consultation
    ...
    const filteredSalesTotal = filteredSales.reduce((sum, o) => sum + o.totalCop, 0);

    // Calculate differences
    const countedCop = parseFloat(countedCash) || 0;
    ...
    const differences = {
        cash_cop: countedCop - salesByMethod.cash_cop,
    ...
    const handleConfirmClose = () => {
        if (!confirm('¿Estás seguro de confirmar el cierre de caja?')) {
            return;
        }
        ...
        const summary = {
            ...
            salesByMethod,
            countedCash: countedCop,
            countedUsd: countedUsdVal,
            countedNequi: countedNequiVal,
            countedDebit: countedDebitVal,
            differences,
            totalDifference,
            observations
        };
