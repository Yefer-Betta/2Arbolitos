...
export const settingsController = {
  ...
  setSetting: asyncHandler(async (req, res) => {
    const { key, value, type = 'string' } = req.body;

    if (!key || value === undefined) {
      throw new AppError('Clave y valor requeridos', 400);
    }
  ...
  setSettings: asyncHandler(async (req, res) => {
  ...
  setAutoStart: asyncHandler(async (req, res) => {
  ...
  getFinanceReport: asyncHandler(async (req, res) => {
  ...
  downloadBackup: asyncHandler(async (req, res) => {
    ...
    if (!dbUrl) throw new AppError('DATABASE_URL no configurada', 500);

    const match = dbUrl.match(/^mysql:\/\/([^:]+):([^@]+)@([^:/]+)(?::(\d+))?\/([^?]+)/);
    if (!match) throw new AppError('DATABASE_URL formato inválido', 500);

    const [, user, password, host, port = '3306', database] = match;

    execFileSync('mysqldump', ['-u', user, `-p${password}`, '-h', host, '-P', port, database], {
