...
router.get('/inventory/:id', authenticate, asyncHandler(async (req, res) => {
  const item = await prisma.inventoryItem.findUnique({ where: { id: req.params.id } });
  if (!item) throw new AppError('No encontrado', 404);
  res.json(item);
}));

router.post('/inventory', authenticate, authorizePermissions(['MANAGE_INVENTORY']), asyncHandler(async (req, res) => {
  const data = req.body;
...
router.patch('/inventory/:id', authenticate, authorizePermissions(['MANAGE_INVENTORY']), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const data = req.body;
...
router.delete('/inventory/:id', authenticate, authorizePermissions(['MANAGE_INVENTORY']), asyncHandler(async (req, res) => {
  const { id } = req.params;
  const before = await prisma.inventoryItem.findUnique({ where: { id } });
